/**
* Name: Menu_in_game.cpp
* Purpose: Class MenuIG
*
* @author Fenris
...
MenuIG::MenuIG(Textures &textures, ScoreDisplay &score_disp, ClockDisplay &clock_disp): select(1000), score_disp(score_disp), clock_disp(clock_disp), frame_on(1),
frame_tick(FRAME_TICK_MAX), str_char_on(0), line_on(1), sprite_drawn(false), display_scores(false)
{
...
void MenuIG::flipFrame(Textures &textures)
{
    if(this->frame_on == 1){
...
void MenuIG::drawLine(sf::RenderWindow &window, const std::string msg, const unsigned x, const unsigned y, const unsigned wait, const unsigned line_num, sf::Sprite *sprite, sf::Color color, unsigned size)
{
    ///Don't do anything until MenuIG isn't done with previous line
...
void MenuIG::draw(sf::RenderWindow &window)
{
    ...
    this->drawLine(window, " R E T U R N", (Globals::SCREEN_WIDTH / 2) - 158, 380, this->PAUSE, 4, nullptr, sf::Color::White, 64);
    ...
    this->drawLine(window, " M E N U", (Globals::SCREEN_WIDTH / 2) - 123, 600, this->PAUSE, 8, nullptr, sf::Color::White, 64);
...
void MenuIG::update(Textures &textures)
{
    if(this->select%5==0)
        this->arrow.setPosition((Globals::SCREEN_WIDTH / 2) - 201, 405);
        ...
        this->arrow.setPosition((Globals::SCREEN_WIDTH / 2) - 166, 625);
